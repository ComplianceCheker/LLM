...
task_string = f"""You are a legal expert, your task to take in {{policies}} and a {{text}}, to identify which policy the text was applicable to, and say the reasoning why you think it's related, if there is no direct connection, just output R99.
Policies: {{
...
peft_model = PeftModel.from_pretrained(model,
                                       'Zephy-7B-CODv2/checkpoint-5054',
                                       torch_dtype=torch.bfloat16,
                                       is_trainable=False)

print(peft_model)

dialogues = test_df[0:10]['dialogue']
peft_model_output= []
for index, dialogue in enumerate(dialogues):
   prompt = f"""
    ...
    df= pd.DataFrame(peft_model_output, columns = ['peft_model_output'])
